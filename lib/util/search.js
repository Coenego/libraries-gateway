 ...
 *
 * @param  {Object}  _parameters   The query parameters
 * @param  {String}  facet         The specified facet name (e.g. format, author,...)
 ...
 */
var createFacetUrl = module.exports.createFacetUrl = function(_parameters, facet, property) {
    var parameters = _.clone(_parameters);
    if (parameters['s.pn']) {
        delete parameters['s.pn'];
    }
        ...
        _.each(parameters, function(value, key) {
            if (_.isArray(value)) {
                _.each(value, function(param) {
                    params.push(util.format('%s=%s', key, param.replace(/&/g, '%26').replace(/\s+/g, '+')));
                });
            } else {
                params.push(util.format('%s=%s', key, value.replace(/&/g, '%26').replace(/\s+/g, '+')));
            }
        });

        // Add the facet property to the array
        property = property.replace(/&/g, '%26').replace(/\s+/g, '+');
        params.push(util.format('s.fvf=%s,%s,false', facet, property));

        // Convert the array to a querystring
...
var createPaginationModel = module.exports.createPaginationModel = function(parameters, pageNumber, pageCount, firstPage, lastPage) {
    ...
    var firstPageObj = new PageModel(qs['s.pn'], 'page', qs, true);
    ...
    var previousPageObj = new PageModel(qs['s.pn'], 'page', qs, visible);
    ...
    var nextPageObj = new PageModel(qs['s.pn'], 'page', qs, visible);

    // Last page
    qs = _createPageQueryString(parameters, lastPage);
    var lastPageObj = new PageModel(qs['s.pn'], 'page', qs, true);
...
var removeTags = module.exports.removeTags = function(snippet) {
    snippet = snippet.replace(/<\/?h>/g, '').trim();
    return snippet;
};

/**
 * @param  {String}   snippet     The string that needs to be linkified
 * @return {String}               The linkified string
 */
var linkify = module.exports.linkify = function(snippet) {
    return snippet.replace(/\s/g,'+').replace(/&/g,'%26').replace(/\,/g,'');
};

/**
 * Function that makes a camel cased string readable for humans
 *
 * @param  {String}   snippet     The string that needs to be converted
 * @return {String}               The converted string
 */
var makeReadable = module.exports.makeReadable = function(snippet, toLowerCase) {
    snippet = snippet.replace(/([a-z](?=[A-Z]))/g, '$1 ');
    return (toLowerCase) ? snippet.toLowerCase() : snippet;
};
