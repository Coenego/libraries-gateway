...
var getFacetsFromResults = module.exports.getFacetsFromResults = function(parameters, callback) {
    ...
    var facetsCollection = ['s.ff=ContentType,or,1,100','s.ff=Language,or,1,100','s.ff=Discipline,or,1,100','s.ff=SubjectTerms,or,1,100'];

    // Predefine some queryString elements
    var queryString = ['s.debug=false'];
...
var getResults = module.exports.getResults = function(parameters, callback) {
    ...
    // Create a collection of facets
    var facetsCollection = ['s.ff=ContentType,or,1,100','s.ff=Discipline,or,1,100','s.ff=Language,or,1,100','s.ff=SubjectTerms,or,1,100'];

    // Predefine some queryString elements
    var queryString = ['s.ps=15', 's.dym=true', 's.debug=false'];
    queryString = queryString.concat(facetsCollection);
...
var _createFacets = function(body, parameters, callback) {
    ...
    var _parameters = _.clone(parameters);
    ...
    try {
        if (body.facetFields) {
            ...
            _.each(body.facetFields, function(facetType) {
                ...
                var moreUrl = search.createFacetMoreUrl(_parameters, facetTypeRawLabel);
                ...
                _.each(facetType.counts, function(facet) {
                    ...
                    var url = search.createFacetUrl(_parameters, facetTypeRawLabel, facetLabel);

                    var facetModel = new FacetModel.Facet(facetLabel, facetAmount, url, facet.isApplied);
...
var _createPagination = function(body, parameters, callback) {
    ...
    try {
        ...
        pagination = search.createPaginationModel(_parameters, pageNumber, pageCount, firstPage, lastPage);
...
var _createSuggestions = function(body, parameters, callback) {
    ...
    // Initialize some variables
    var originalQuery = parameters['s.q'];
    var suggestedItems = [];
            ...
            if (body.didYouMeanSuggestions.length) {
                ...
                _.each(body.didYouMeanSuggestions, function(suggestion) {
                    if (suggestion.suggestedQuery) {
                        ...
                        _parameters['s.q'] = suggestion.suggestedQuery;

                        var label = _parameters['s.q'];
                        var url = qs.stringify(_parameters);
