...
var ResourcesController = module.exports.ResourcesController = function() {
    ...
    that.getContent = function(req, res) {
        ...
        var parameters = {
            'data': {
                'disciplines': config.constants.disciplines,
                'contenttypes': config.constants.contenttypes,
        ...
        // Render the results after a query has been executed
            ...
            SearchAPI.getResults(req.query, function(err, results) {
                ...
                try {
                    ...
                    var itemTemplate = 'partials/find-a-resource/api-results-summon';
    ...
    that.getResourceDetail = function(req, res) {
        ...
        if (!id) {
            log().error({'code': 400, 'msg': 'Invalid id specified'});
            return that.renderTemplate(req, res, null, 'errors/400', 'error-400');
        }

        // Fetch the results from the API
        SearchAPI.getResultById({'id': id}, function(err, results) {
            if (err) {
            ...
            try {
                ...
                var shareUrl = config.server.protocol + '://' + config.server.host + ':' + config.server.port + '/find-a-resource/' + data.resource.ID;
                ...
                var template = 'partials/resource-detail/summon';
    ...
    that.getFacetsForResults = function(req, res) {
        ...
        if (!req.query.facet || !req.query['q']) {
            return that.renderTemplate(req, res, null, 'errors/400', 'error-400');
        }
